...
const serveFileContents = (filePath: string, contentType: string, response: CustomResponse) => {
...
const server = http.createServer((req: CustomRequest, res: CustomResponse) => {
  ...
  if (pathname === '/') {
    serveFileContents(path.join(__dirname, 'public', 'index.html'), 'text/html', res);
  } else if (pathname === '/api/data') {
    // Example API response
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ message: 'Hello from the Node.js server!', timestamp: Date.now() }));
  } else {
    ...
    serveFileContents(path.join(__dirname, 'public', pathname), contentType, res);
  }
});
